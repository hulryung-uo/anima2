...
(planned)" list): mutation operators stay within the config space (seeded,
deterministic), THE CONVERGENCE TEST (a stubbed `eval_fn` landscape proving
`evolve()` concentrates the archive toward the best region faster than
`random_search()` under the same budget — the offline analogue of the live
comparative gate), the kill switch halting between evals, and the
`max_genomes` cap. No live server anywhere in this file — every eval is a
pure-Python stub matching `evolve.py::EvalFn`'s signature.
"""
...
def test_op_profession_always_in_scenario_supported_professions():
    rng = random.Random(5)
    g = _base_genome(profession="miner")
    child = evolve.op_profession(g, rng)
    assert child.profession in evolve.PROFESSION_SCENARIO
    # Only one candidate exists today (see evolve.py's own docstring) — the
    # operator degrades to a same-value "swap" rather than raising, which
    # this test pins down explicitly so a future second candidate is a
    # deliberate, noticed change to this test, not a silent behavior shift.
...
def test_convergence_evolve_concentrates_toward_the_best_region(tmp_path):
...
def test_convergence_evolve_beats_random_search_baseline_same_budget(tmp_path):
