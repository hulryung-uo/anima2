...
#: `MineAndSmelt`, `MineSmeltDeliver`, `SpeakPending`, `Wander` — 14 concrete
#: `Skill` subclasses; `Skill`/`SkillContext`/`SkillResult`/`Status`/`Goal`
#: are also exported but aren't skills, so the registry doesn't cover them).
