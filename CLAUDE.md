...
- **Autonomy thread** ([`docs/AUTONOMY-ROADMAP.md`](docs/AUTONOMY-ROADMAP.md)): the
  immutable capability registry + closed capability cognition (B3/B4), and four
  autonomous per-profession "Lives" on one orchestrator (`anima2/warrior_life.py` base;
  mage/woodsman/carpenter subclasses) — each hunts/works, restocks, banks, and
  ...
  telemetry — every clause a live-caught lesson; see `docs/AUDIT-2026-07-29.md`).
- **Two live inter-agent supply chains**, coordinating only through items on the
  ground: tinker→mage (gold), lumberjack→carpenter (boards). Economics warning: at
...
gold-per-life fitness means something. Do NOT burn a multi-hour single-GM live budget on the rerun before that; a
